      ...
      >
        ...
        <el-form-item label="API Key">
          <el-input
            ...
            :placeholder="form.api_key_configured ? '已保存 API Key；留空则不修改' : '请输入 API Key'"
          />
          <div class="form-tip">
    ...
    <el-card class="status-card" shadow="never">
      ...
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="测试信息">
          {{ form.last_test_message || '暂无测试信息' }}
        </el-descriptions-item>

        <el-descriptions-item label="测试时间">
          {{ form.last_test_at || '暂无测试时间' }}
        </el-descriptions-item>
      </el-descriptions>
    </el-card>
  </div>
...
const saveConfig = async () => {
  ...
  try {
    ...
    copyConfigToForm(res.config || {})
    ElMessage.success(res.message || 'AI 审查配置已保存')
  } catch (error) {
    ElMessage.error(getErrorMessage(error, '保存 AI 审查配置失败'))
  } finally {
    saving.value = false
  }
}
...
const testConfig = async () => {
  ...
  try {
    ...
    ElMessage.success(res.message || 'AI 服务连通性测试成功')
  } catch (error) {
    ElMessage.error(getErrorMessage(error, 'AI 服务连通性测试失败'))
  } finally {
    testing.value = false
    await loadConfig()
  }
