<template>
  <div class="page-container">
    ...
    <el-card class="filter-card">
      <el-form :model="filters" class="filter-form" label-width="48px">
        ...
        <el-form-item label="搜索" class="filter-search-item">
          <el-input
      ...
      <el-table :data="userList" v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="role" label="角色" width="100">
          <template #default="{ row }">
            <el-tag :type="roleType[row.role] || 'info'" size="small">
              {{ roleMap[row.role] || row.role || '未知' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="350" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              v-if="row.status === 'pending_verification' || row.status === 'rejected'"
              link
              type="success"
    ...
    <el-dialog v-model="editDialogVisible" title="编辑用户" width="500px">
      <el-form :model="editForm" label-width="100px">
        ...
        <el-form-item label="角色">
          <el-select v-model="editForm.role" class="edit-select">
            <el-option label="学生" value="student" />
            <el-option label="导师" value="teacher" />
            <el-option label="专家" value="expert" />
            <el-option label="管理员" value="admin" />
...
const fetchData = async () => {
  ...
  try {
    ...
    pagination.limit = Number(response.pagination?.limit || pagination.limit)
  } catch (error) {
...
const editUser = (row) => {
  ...
  editForm.quota_total = Number(row.quota_total || 0)
}
...
const rejectUser = async (row) => {
  try {
    ...
    if (error !== 'cancel' && error !== 'close') {
      ElMessage.error(error.error || '驳回失败')
    }
  }
}
